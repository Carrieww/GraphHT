...
def get_state_plot(g, num_edges, output_filename="state_bar.png"):
    img_output_path = os.path.join("StatisticsAnalysis", dataset_name, output_filename)
    ...
    condition_dict = []
    for i in labels:
        condition_dict.append({"business": {"state": i}})

    year_num_list = get_list(g, condition_dict)

    # Bar chart
    plot_bar_chart(
        year_num_list, labels, num_edges, "State", img_output_path=img_output_path
    )
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(
        os.path.join(os.getcwd(), "StatisticsAnalysis", "yelp_dataset", "state_bar.png")
    ):
        print("start preparing the bar chart for different states.")
        get_state_plot(graph, num_edges)
