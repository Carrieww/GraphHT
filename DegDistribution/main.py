...
def main():
    ...
    elif args.dataset == "ca_GrQc":  # not one component
...
def sampling_init(args, graph):
    ori_result_list = deg_dist(args, graph)
    plt.plot(
        np.linspace(1, args.n_deg + 1, args.n_deg),
        ori_result_list,
        label="original degree dist",
    )
    ...
    for ratio in args.ratio_list:
        ...
        if args.sampling_method == "SRW":
            ...
            if args.with_weight_func == True:
                ...
                args.result[ratio] = deg_dist(args, new_graph)
        ...
        plt.plot(
            np.linspace(1, args.n_deg + 1, args.n_deg),
            args.result[ratio],
            label=f"degree dist ({round(ratio,4)})",
        )
    ...
    plt.ylabel("pdf")
    plt.legend()
    plt.title(f"degree distribution ({args.dataset}) - {args.sampling_method}")
...
def deg_dist(args, new_graph):
    new_degreeView = new_graph.degree()
    result = []
    for d in range(1, args.n_deg + 1):
        ori_numerator_sum = 0
        for i in new_graph.nodes():
            if new_degreeView[i] == d:
                ori_numerator_sum += 1
        result.append(ori_numerator_sum)
    result = [x / new_graph.number_of_nodes() for x in result]
    return result
